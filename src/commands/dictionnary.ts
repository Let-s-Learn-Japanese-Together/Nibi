...
async function detectAndTranslate(
  word: string,
  targetLang: "ja" | "fr" | "en" = "ja",
  allowedSourceLangs?: string[],
) {
  ...
  try {
    ...
    if (
      allowedSourceLangs &&
      !allowedSourceLangs.includes(result.from.language.iso)
    ) {
      // Forcer la langue à 'en' par défaut si non détectée correctement
      const forcedLang =
        allowedSourceLangs.includes("fr") && isFrenchWord(word) ? "fr" : "en";
      return {
        original: word,
...
async function getKanaForms(
  text: string,
): Promise<{ hiragana: string; katakana: string }> {
  try {
...
const dictionary_cmd: Command = {
  data: {
    options: [
      {
        type: 1,
        name: "to-japanese",
        description: "Translate French or English word to Japanese with romaji",
        options: [
          {
            type: 3,
            name: "word",
            description: "French or English word to translate to Japanese",
            required: true,
          },
        ],
      },
      {
        type: 1,
        name: "from-japanese",
        description: "Translate from Japanese to French or English",
        options: [
          {
            type: 3,
            name: "word",
            description: "Japanese word (hiragana, katakana, kanji, or romaji)",
            required: true,
          },
          {
            type: 3,
    ...
    name: "dictionary",
    description: "Translate words between French, English and Japanese",
    type: 1,
  },

  async execute(interaction: Interaction) {
    const subcommandOption = interaction.data?.options?.find(
      (opt) => opt.name === "to-japanese" || opt.name === "from-japanese",
    );
          ...
          embeds: [
            {
              ...
              description:
                'Subcommand not found. Please use "to-japanese" or "from-japanese".',
            },
          ],
        },
      };
              ...
              embeds: [
                {
                  ...
                  description:
                    "Unable to translate the word. Please try again.",
                },
              ],
            },
          };
              ...
              embeds: [
                {
                  ...
                  description:
                    "This command only supports French and English words.",
                  fields: [
                    {
